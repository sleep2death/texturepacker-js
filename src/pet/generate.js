  ...
  fs.readdir(`${root}/${path}`, (err, files) => {
    ...
    if(files.length > 7){
		...
		if(hasAlpha){
	      ...
	      }else if(ACTION_A.indexOf(file) >= 0){
	      	createSprite(root, output, {path: `${path}/${file}`, name: file, hasAlpha: true, isAlpha: true}, next)	
	        bar.tick({input: `${root}/${path}`, output: `${output}/${p}/${file}`})
	      }else{
	        next()
	      }
...
function createSprite(root, output, options, next) {
  ...
  mkdirp(`${output}/${p}`, err => {
    ...
    mkdirp(`${output}/${p}/a`, error => {
      ...
      pack(`${root}/${options.path}`, {output: outPath, name: options.name, hasAlpha: options.hasAlpha}, next)
    })
  })
}
